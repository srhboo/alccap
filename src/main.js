...
function drawCurves(parent) {
  ...
  for (let i = 0; i < curvePoints.length; i++) {
    ...
    console.log(c1x, c1y, c1z, p1x, p1y, p1z, p2x, p2y, p2z, c2x, c2y, c2z);
...
function setupLines() {
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    const lm = (l[3] - l[1]) / (l[2] - l[0]);
    const lb = l[1] - lm * l[0];
    lines.push({ l, plane: "xz" });
    vertLines2.push({ m: lm, b: lb, x: topX });
  }
...
function drawGrid(parent, z, topOffset = 0) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (plane == "xy") {
      ...
      points.push(new THREE.Vector3(x1, -500, y1));
      points.push(new THREE.Vector3(x2, -500, y2));
    }
